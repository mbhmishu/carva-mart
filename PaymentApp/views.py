...
from PaymentApp.forms import BillingForm,OrderMsgForm
...
@login_required
def order_msg(request ,pk):

    item = get_object_or_404(Product, pk=pk)
    order = Order.objects.filter(user=request.user, ordered=False).first()

    saved_address = OrderMsg.objects.filter(user=request.user,order=order,item=item).first()

    if not saved_address:
        saved_address = OrderMsg.objects.get_or_create(user=request.user,order=order,item=item)
        saved_address = saved_address[0]
        saved_address.is_creat=True
        saved_address.save()

    form = OrderMsgForm(instance=saved_address)
    if request.method == "POST":
        form = OrderMsgForm(request.POST, request.FILES, instance=saved_address)
        if form.is_valid():
            form.save()
            form = OrderMsgForm(instance=saved_address)
            messages.success(request, f"Product propartis Saved!")
            return HttpResponseRedirect(reverse("App_Shop:home"))
    
    return render(request, 'App_Payment/ordermsg.html', context={"form":form,"saved_address":saved_address})
...
def payment(request):
    ...
    if not request.user.user_info.is_fully_filled():
    ...
    mypayment.set_customer_info(name=current_user.user_info.full_name, email=current_user.email, address1=current_user.user_info.address_1, address2=current_user.user_info.address_1, city=current_user.user_info.city, postcode=current_user.user_info.zipcode, country=current_user.user_info.country, phone=current_user.user_info.phone)

    mypayment.set_shipping_info(shipping_to=current_user.user_info.full_name, address=saved_address.address, city=saved_address.city, postcode=saved_address.zipcode, country=saved_address.country)
